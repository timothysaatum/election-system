...
const TokenGenerator = ({ electorates = [], onUpdate }) => {
  ...
  const filteredElectorates = useMemo(() =>
    electorates.filter((e) => {
      const searchTarget = [e.student_id, e.name, e.full_name, e.program]
    ...
    <div className="p-2">
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Token Generation</h1>
            <p className="text-gray-600 mt-1">Generate voting tokens for electorates</p>
          </div>
          {onUpdate && (
            <button
              onClick={onUpdate}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-blue-900">{electorates.length}</p>
          </div>
          <div className="bg-green-50 rounded-lg p-4">
            <p className="text-sm text-green-600 font-medium">Tokens Generated</p>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            <p className="text-sm text-purple-600 font-medium">Already Voted</p>
          ...
          <div className="bg-white rounded-lg shadow-sm">
            ...
            <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto">
              {filteredElectorates.length === 0 ? (
                <div className="p-8 text-center text-gray-500">No voters found</div>
              ) : (
                filteredElectorates.map((electorate) => (
                  <div key={electorate.id} className="p-4 hover:bg-gray-50 transition">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {electorate.has_voted && (
                            <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-full">Voted</span>
                          )}
                          {electorate.voting_token && !electorate.has_voted && (
                            <span className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full">Has Token</span>
                          )}
                        </div>
                      ...
                      <button
                        ...
                        disabled={generatingFor === electorate.id || electorate.has_voted}
                      ...
                      >
                        {generatingFor === electorate.id ? (
                          ...
                          <><Key className="h-4 w-4" />{electorate.voting_token ? "Regenerate" : "Generate"}</>
                        )}
                      </button>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg shadow-sm sticky top-6">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600 mt-1">Write these tokens for voters</p>
            </div>
            <div className="divide-y divide-gray-200 max-h-[600px] overflow-y-auto">
                  ...
                  <div key={index} className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50">
                    <div className="mb-3">
                      <p className="font-semibold text-gray-900">
                        {item.electorate.name || item.electorate.full_name || item.electorate.student_id}
                      </p>
                      <p className="text-sm text-gray-600">{item.electorate.student_id}</p>
                    </div>
                    <div className="bg-white rounded-lg p-4 border-2 border-blue-300">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs font-medium text-gray-600">VOTING TOKEN</p>
                        <button onClick={() => handleCopyToken(item.token)} className="text-blue-600 hover:text-blue-700">
                          {copiedToken === item.token ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                        </button>
                      </div>
                      <p className="text-2xl font-bold text-gray-900 tracking-wider font-mono">{item.token}</p>
                    </div>
                    <div className="mt-3 flex items-center gap-2 text-xs text-gray-500">
                      <AlertCircle className="h-3 w-3" />
                      <span>Generated {new Date(item.timestamp).toLocaleTimeString()}</span>
                    </div>
                  </div>
                ))
              )}
