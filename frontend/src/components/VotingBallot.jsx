...
const VotingBallot = ({ voterData, onVoteComplete, sessionTime, onSessionEnd }) => {
  ...
  // portfolio_id and candidate_id are UUIDs — never parseInt them.
  // parseInt("3f2504e0-...") === NaN, which the backend rejects.
  const handleFinalSubmit = async () => {
    const finalVotes = Object.entries(selectedVotes).map(([portfolio_id, candidate_value]) => {
      if (candidate_value === "reject") {
        // abstain — still needs a valid candidate_id, use the first in portfolio
        const portfolio = portfolios.find((p) => String(p.id) === String(portfolio_id));
        const candidate_id = portfolio?.candidates[0]?.id;
        return { portfolio_id, candidate_id, vote_type: "abstain" };
      }

      // candidate_value is already the candidate's UUID string
      return {
        portfolio_id,
  ...
  // FIX #4 — safe session time display (guards against NaN)
                  ...
                  <div className="text-center">
                    ...
                    <div className="flex flex-wrap justify-center gap-6">
                      <button
                        ...
                        className={`px-12 py-5 rounded-2xl font-black text-2xl transition-all shadow-lg ${currentSelection === currentPortfolio.candidates[0].id
                      ...
                      <button
                        ...
                        className={`px-12 py-5 rounded-2xl font-black text-2xl transition-all shadow-lg ${currentSelection === "reject"
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl p-10 max-w-lg w-full shadow-2xl border border-slate-100">
            ...
            <p className="text-slate-500 mb-8 font-medium text-center">
              You are about to submit your ballot. Please review your selections carefully before
              confirming.
            </p>
              ...
              <div className="space-y-3">
                {portfolios.map((portfolio) => (
                  <div key={portfolio.id} className="flex justify-between items-center text-sm">
                    ...
                    <span
                      className={`font-bold ${selectedVotes[portfolio.id] ? "text-slate-900" : "text-slate-400 italic"
                        }`}
                    >
                      {selectedVotes[portfolio.id] === "reject"
                        ...
                        : selectedVotes[portfolio.id]
                          ? portfolio.candidates.find(
                            (c) => String(c.id) === String(selectedVotes[portfolio.id])
                          )?.name
                          : "Not selected"}
                    </span>
                  </div>
